        ...
        <section class="section">
            <div class="row  row-gap-2 mb-4">
                @isset($properties)
                    @if ($properties != null)
                        @php
                            $property_data = $properties->load('propertyImage');
                        @endphp
                                            ...
                                            <div class="user-image rounded border-2 border border-primary">
                                                <a href="{{ route('property.show', [$property->id]) }}">
                                                    <img src="{{ isset($property->propertyImage[0]->name) && !empty($property->propertyImage[0]->name) && check_file($property->propertyImage[0]->name) ? get_file($property->propertyImage[0]->name) : asset('packages/workdo/PropertyManagement/src/Resources/assets/upload/thumbnail-not-found.png') }}"
                                                        alt="PropertyImage" id="thumbnail" class="h-100 w-100">
                                                </a>
                                            </div>
                                            <div class="user-content">
                                                <h4 class="mb-2"> <a
                                                        href="{{ route('property.show', $property->id) }}">{{ $property->name }}</a>
                                                </h4>
                                                <span class="text-dark text-md">
                                                    <i class="ti ti-map-pin"></i> {{ $property->address }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card-body  text-center p-3">
                                        <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                                @permission('property show')
                                                    <a class="btn btn-sm border"
                                                        ...
                                                        data-title="{{ __('Property Details') }}">
                                                ...
                                                @if (\Auth::user()->type != 'tenant')
                                                    @permission('property edit')
                                                        <a data-ajax-popup="true" data-size="lg"
                                                            data-title="{{ __('Edit Property') }}"
                                                            data-url="{{ route('property.edit', [$property->id]) }}"
                                                    ...
                                                    @permission('property delete')
                                                        {{ Form::open(['route' => ['property.destroy', $property->id], 'class' => 'm-0']) }}
                                                        @method('DELETE')
                                                        <a href="#!" aria-label="Delete"
                                                            data-confirm="{{ __('Are You Sure?') }}"
                                                            data-text="{{ __('This action can not be undone. Do you want to continue?') }}"
                                                            data-confirm-yes="delete-form-{{ $property->id }}"
                                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                            ...
                                            <div class="d-flex flex-column">
                                                <span class="text-dark text-md">{{ __('Total Unit') }} :
                                                    {{ $property->propertyUnitCount() }}</span>
                                                <span class="text-dark text-md">{{ __('Available') }} :
                                                    {{ $property->availablePropertyUnit() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                @if (\Auth::user()->type != 'tenant')
                    @auth('web')
                        @permission('property create')
                            <div class="col-xxl-3 col-xl-4 col-md-6">
                                <a class="btn-addnew-project border-primary" data-ajax-popup="true" data-size="lg"
                                    data-title="{{ __('Create Property') }}" data-bs-toggle="tooltip" title="{{ __('Create') }}" data-url="{{ route('property.create') }}">
                                    <div class="bg-primary proj-add-icon">
                                        <i class="ti ti-plus my-2"></i>
                                    </div>
                                    <h6 class="my-2 text-center">{{ __('Create Property') }}</h6>
                                    <p class="text-muted text-center">{{ __('Click here to Create Property') }}</p>
                                </a>
                            </div>
                        @endpermission
                    @endauth
            ...
            {!! $properties->links('vendor.pagination.global-pagination') !!}
        </section>
    </div>
@endsection
