...
@section('page-action')
    <div class="d-flex">
        @stack('addButtonHook')
        <a href="{{ route('tenant.index') }}" class="btn btn-sm btn-primary btn-icon me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
            <i class="ti ti-list text-white"></i>
        </a>
        @permission('tenant create')
            <a class="btn btn-sm btn-primary" data-ajax-popup="true" data-size="lg" data-title="{{ __('Create Tenant') }}"
                data-url="{{ route('tenant.create') }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Create') }}">
                    ...
                    <div class="card-header border border-bottom p-3 h-100">
                        <div class="user-img-wrp d-flex align-items-center">
                            <div class="user-image rounded border-2 border border-primary">
                                <a href="{{ check_file($tenant->avatar) ? get_file($tenant->avatar) : 'uploads/users-avatar/avatar.png' }}"
                                    target="_blank">
                                    <img src="{{ check_file($tenant->avatar) ? get_file($tenant->avatar) : 'uploads/users-avatar/avatar.png' }}"
                            ...
                            <div class="user-content">
                                @permission('tenant show')
                                    <a href="{{ route('tenant.show', \Crypt::encrypt($tenant['id'])) }}">
                                        <h4 class="mb-2 text-break">{{ !empty($tenant->name) ? $tenant->name : '' }}</h4>
                                    </a>
                                @else
                                    <h4 class="mb-2 text-break">{{ !empty($tenant->name) ? $tenant->name : '' }}</h4>
                                @endpermission
                                <span class="text-dark text-md">{{ !empty($tenant->email) ? $tenant->email : '' }}</span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @if (Laratrust::hasPermission('tenant show') ||
                                    ...
                                    @if($tenant['lease_end_date'] != null && $tenant['lease_end_date'] < \Carbon\Carbon::today())
                                        @permission('tenant show')
                                            <a href="#!" data-url="{{ route('tenant.status', $tenant['id']) }}"
                                                data-ajax-popup="true" data-size="md" data-title="{{ __('Tenant Status') }}"
                                                data-bs-original-title="{{ __('Action') }}" data-bs-toggle="tooltip"
                                                data-bs-placement="top" class="btn btn-sm border">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M5.45405 15C5.37838 15 5.3027 15 5.3027 15C5.15135 14.9188 5 14.7564 5 14.5128V1.52083C5 1.35843 5.07568 1.11483 5.3027 1.03363C5.45405 0.952434 5.68108 1.03363 5.83243 1.11483L11.8865 7.61082C12.0378 7.77322 12.0378 8.09802 11.8865 8.26042L5.83243 14.7564C5.68108 14.9188 5.60541 15 5.45405 15ZM5.90811 2.65763V13.2948L10.9027 7.93562L5.90811 2.65763Z" fill="#060606"/>
                                                    </svg>
                                            </a>
                                        @endpermission
                                    @endif
                                    @permission('tenant show')
                                            <a href="{{ route('tenant.show', \Crypt::encrypt($tenant['id'])) }}"
                                                data-title="{{ __('View') }}" data-bs-original-title="{{ __('View') }}"
                                    ...
                                    @permission('tenant edit')
                                        <a href="#!" data-url="{{ route('tenant.edit', $tenant['id']) }}"
                                            data-ajax-popup="true" data-size="lg" data-title="{{ __('Edit Tenant') }}"
                                    ...
                                    @if (!empty($tenant['tenant_id']))
                                        @permission('tenant delete')
                                            ...
                                            <a href="#!" aria-label="Delete" data-confirm="{{ __('Are You Sure?') }}"
                                                ...
                                                data-confirm-yes="delete-form-{{ $tenant->id }}"
        ...
        @permission('tenant create')
            <div class="col-xxl-3 col-xl-4 col-md-6">
                <a href="#" class="btn-addnew-project border-primary p-4" data-ajax-popup="true" data-size="lg"
                    data-title="{{ __('Create Tenant') }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Create') }}" data-url="{{ route('tenant.create') }}">
                    <div class="bg-primary proj-add-icon">
                        <i class="ti ti-plus my-2"></i>
                    </div>
                    <h6 class="my-2 text-center">{{ __('Create Tenant') }}</h6>
                    <p class="text-muted text-center mb-0">{{ __('Click here to Create Tenant') }}</p>
                </a>
            </div>
        @endpermission
    </div>
    {!! $tenants->links('vendor.pagination.global-pagination') !!}
@endsection
